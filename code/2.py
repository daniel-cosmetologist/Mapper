...
############################
# 1. Функция для условного форматирования DataTable (окраска по 5-му и 95-му процентилю)
############################
...
############################
df_path = "../datasets/db_nl_preprocessed-edit.csv"  # Задайте корректный путь к файлу CSV
...
# Создаём все виды как отдельные Div-блоки, каждый с уникальным id (нет повторов!)
...
def update_cytoscape(selected_filters, mapper_store):
    ...
    # Если фильтр пуст или выбран "all", то используем все группы – раскраска по target
    ...
    new_elems = []
    # Для каждого узла используем жёсткую раскраску: если target=1 (больше или равно) → pink, иначе → cyan
    ...
    # Добавляем рёбра из кэшированных базовых элементов

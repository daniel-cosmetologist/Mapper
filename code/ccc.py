...
def build_single_mapper(df, features, cover_params, n_clusters=3):
    # Переводим данные в numpy массив
    ...
    # Формируем базовые элементы узлов и рёбер для Cytoscape
    elements = []
    for nid in G['nodes']:
        elements.append({
...
############################
# Загрузите данные из CSV (путь корректный)
df_path = "../datasets/db_nl_preprocessed-edit.csv"
...
# Сохраняем глобальный маппинг для таблицы
...
# Применяем цвета к базовым элементам (узлы)
...
# Список числовых признаков (исключая target и пол)
numeric_features = df.select_dtypes(include=[np.number]).columns.tolist()
numeric_features = [f for f in numeric_features if f not in ['target', 'пол']]

############################
# 8. Интерфейс Dash с вкладками и dcc.Store для кэширования графа
############################
app = dash.Dash(__name__)
server = app.server

# Сохраним кэшированные данные (base_elements, node_data и JSON-представление датафрейма)
mapper_store_data = {
...
# Компонент загрузки файла (пока используется только для информирования, данные загружаются по умолчанию)
...
# Cytoscape‑компонент с изменёнными параметрами (тонкие, пунктирные рёбра; рамка; поддержка множественного выделения)
cyto_graph = cyto.Cytoscape(
    id='cytoscape-graph',
    elements=base_elements,   # первичная структура, кэшируется в store
    stylesheet=[
        {'selector': 'node', 'style': {'label': 'data(label)', 'font-size': '1px'}},
        {'selector': 'node:selected', 'style': {'background-color': 'blue'}},  # выделенные узлы – ярко-синие
        {'selector': 'edge', 'style': {
...
app.layout = html.Div([
    ...
    store_component,
    upload_component,  # Пока данные загружаются по умолчанию
...
############################
# 9. Callback для обновления окраски узлов (граф не перестраивается, только меняется цвет)
############################
@app.callback(
...
def update_cytoscape(selected_filters, mapper_store):
    # Получаем сохранённые данные из Store
    ...
    new_elems = []
    # Для каждого узла используем фиксированную раскраску: target=1 -> pink, target=0 -> cyan.
    for nid, info in cached_node_data.items():
        # Здесь жёстко: если количество target=1 >= target=0, то pink, иначе cyan.
...
############################
# 10. Callback для обновления таблиц по выбранным узлам
############################
@app.callback(
...
def update_tables(selected_nodes, mapper_store):
    if mapper_store is None:
        raise dash.exceptions.PreventUpdate
    # Восстанавливаем датафрейм из хранилища
    df_json = mapper_store.get("df_json", None)
    if df_json is None:
        raise dash.exceptions.PreventUpdate
    df_cached = pd.read_json(df_json, orient='split')
    ...
    for nd in selected_nodes:
        nid = nd.get('id')
        if nid in mapper_store.get("node_data", {}):
            all_indices.extend(mapper_store["node_data"][nid].get('members', []))
...
def update_feature_hist(feature, mapper_store):
    if mapper_store is None:
        raise dash.exceptions.PreventUpdate
    # Используем кэшированный датафрейм
    df_json = mapper_store.get("df_json", None)
    if df_json is None or feature is None or feature not in pd.read_json(df_json, orient='split').columns:
        return go.Figure()
    df_cached = pd.read_json(df_json, orient='split')
...
############################
# 13. Callback для переключения набора столбцов DataTable (basic/full)
############################
@app.callback(
...
############################
# 14. Инициализация кэша (dcc.Store)
############################
# При запуске приложения сразу сохраняем данные (поскольку загрузка файла не требуется)
mapper_store_data = {
    "base_elements": base_elements,
    "node_data": node_data,
    "df_json": df.to_json(date_format='iso', orient='split')
...
############################
app.layout.children.append(dcc.Store(id="mapper-store", data=mapper_store_data))

...
def build_single_mapper(df, features, cover_params, n_clusters=3):
    ...
    # Формируем базовые элементы (узлы и рёбра) для Cytoscape
    elements = []
    for nid in G['nodes']:
        elements.append({
    ...
    # Собираем дополнительную информацию по узлам: считаем пациентов с target=1 и target=0
    node_data = {}
...
############################
df_path = "../datasets/db_nl_preprocessed-edit.csv"  # укажите корректный путь к CSV
...
############################
# 4. Фиксированные цвета для узлов
############################
# Если количество пациентов с target=1 >= target=0 → pink, иначе → cyan.
def get_node_color(info):
    return "pink" if info['target1_count'] >= info['target0_count'] else "cyan"

# Обновляем базовые элементы: устанавливаем цвет узла
...
############################
# 8. Инициализация кэша (dcc.Store)
############################
# Кэшируем базовые элементы, node_data и датафрейм
mapper_store_data = {
...
# Настраиваем Cytoscape: узлы с маленькими пунктирными рёбрами, рамка; поддержка множественного выделения.
cyto_graph = cyto.Cytoscape(
    id='cytoscape-graph',
    elements=base_elements,  # Граф построен один раз
    stylesheet=[
        {'selector': 'node', 'style': {
            'label': 'data(label)',
            'font-size': '1px'
    ...
    layout={
        'name': 'cose',
        'animate': False,       # не анимируем при каждом новом рендере
        'randomize': False,
        'nodeRepulsion': 3000,
        'idealEdgeLength': 40,
        'numIter': 500,
...
app.layout = html.Div([
    ...
    store_component,  # Хранит кэшированные данные
    upload_component,  # Компонент загрузки файла (пока данные по умолчанию)
...
############################
# 9. Callback для обновления цвета узлов (без перестройки графа)
############################
@app.callback(
...
def update_cytoscape(selected_filters, mapper_store):
    ...
    new_elems = []
    # Для каждого узла задаём фиксированную раскраску: если target=1 (patients с target=1 больше или равно) → pink, иначе → cyan.
...
############################
# 11. Callback для обновления гистограммы признака
############################
@app.callback(
...
############################
# 12. Callback для обновления гистограммы узлов
############################
@app.callback(

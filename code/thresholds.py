thresholds = {
    ...
    'пол': {'low': 0, 'high': 1},  # 0 – женщины, 1 – мужчины
    'масса_тела': {'low': 50, 'high': 90},  # кг, как ориентир для взрослой популяции

    # Параметры, связанные с композиционным анализом тела, адаптированы под клиническую популяцию:
    'жм': {'low': 15, 'high': 40},        # жировая масса в кг (у мужчин ~15–30, у женщин ~20–40)
    'мм': {'low': 20, 'high': 45},        # мышечная масса в кг (примерные диапазоны)
    'ож': {'low': 10, 'high': 35},        # общий жировой процент (в %; нормальные значения – 10–25% у мужчин, 20–35% у женщин)
    'тощая_мт': {'low': 70, 'high': 85},   # тощая масса тела как процент от общей массы (70–85% у здоровых)
    'акм': {'low': 40, 'high': 70},        # активная клеточная масса в кг (иногда 40–70 кг, зависит от состава тела)

    # Остальные параметры можно оставить по исходным значениям или откорректировать,
    # если имеются дополнительные сведения по единицам измерения:
    'рост': {'low': 150, 'high': 200},      # в см
